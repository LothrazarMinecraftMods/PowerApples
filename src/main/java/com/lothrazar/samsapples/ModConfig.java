...
public class ModConfig{
	...
	static void loadConfig(Configuration cfg){
		...
		ItemRegistry.apple_chorus_enabled = cfg.get(category, "apple_chorus_enabled", true).getBoolean();

		category = "recipes";
		cfg.addCustomCategoryComment(category, "True means you have to fully surround the apple with 8 items, false means only a single item will craft with the red apple.");
		...
		ItemRegistry.apple_chorus_expensive = cfg.get(category, "apple_chorus_expensive", false).getBoolean();

		category = "effect_tweaks";

		PotionRegistry.slowfallSpeed = cfg.getFloat("slowfall_speed", category, 0.41F, 0.1F, 1F, "This factor affects how much the slowfall effect slows down the entity.");

		category = "effect_ids";

		cfg.addCustomCategoryComment(category, "IDs are only exposed to avoid conflicts with other mods.  Messing with these might break the game.   ");
